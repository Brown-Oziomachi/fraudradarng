...
/* ============================================================
   Exit-intent check — "before you send that transfer".
...
<template>
  ...
  <!-- ============ FRAUD FILES — deep-dive accordion (own chunk, hydrates when scrolled into view) ============ -->
  <div class="reveal">
    <LazyFraudFilesAccordion hydrate-on-visible />
  </div>
  ...
  <section class="registry-context reveal">
    ...
    <LazyThreatRegistryPromo hydrate-on-visible />
  </section>

  <!-- ============ ANNOTATED SCAM MODAL — own chunk, only fetched once opened ============ -->
  <LazyScamAnnotationModal :open="scamModalOpen" @close="closeScamModal" />

  <!-- ============ EXIT-INTENT MODAL — own chunk, only fetched once opened ============ -->
  <LazyExitIntentModal :open="exitModalOpen" @close="closeExitModal" />

  <!-- ============ LIVE-SIGNAL TOASTS — own chunk, only fetched once shown ============ -->
  <LazyLiveSignalToast v-if="showToast" :title="tickerItems[0]" :report-id="validReports[0]?.id"
    @dismiss="dismissToast" />
  <LazyLiveSignalToast v-if="showToast" :title="tickerItems[1]" :report-id="validReports[1]?.id"
    @dismiss="dismissToast" />
</template>
